...
public:
	...
	inline const T* data() const {return (const double*)pData;};
	inline T operator [](int index) const{return pData[index];};
	inline T& operator[](int index) {return pData[index];};
	inline T data(int row,int col)const {return pData[row*nCol+col];};
	inline T& data(int row,int col) {return pData[row*nCol+col];};
